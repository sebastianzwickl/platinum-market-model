...
def get_demand_data_for_both_markets(_demand=None):
    ...
    for _year in range(2025, 2051, 1):
        if _year < 2030:
            ...
            _dict_demand_EU[_market, _year] = _v20xx
        elif _year == 2030:
            _v2030 = _demand_EU.loc[_demand_EU.Year == 2030]["Value"].item()
            _dict_demand_EU[_market, _year] = _v2030
        else:
            _v2030 = _demand_EU.loc[_demand_EU.Year == 2030]["Value"].item()
...
def add_decision_variables_vector_x(model=None):
    ...
    model.var_q_arb = py.Var(model.set_t, doc="VAR: (q_arb_M1_M2_t).")
