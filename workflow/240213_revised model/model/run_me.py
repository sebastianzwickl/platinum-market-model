...
model.comp_11_1 = py.Constraint(
    model.set_e, model.set_market, model.set_t, rule=comp_11_1
)
model.comp_11_2 = py.Constraint(
    model.set_e, model.set_market, model.set_t, rule=comp_11_2
)
...
model.objective = py.Objective(expr=dummy_obj, sense=py.minimize)
...
for _year in model.set_t:
    _value = (
        model.var_mu_12["Russia", "M2", _year]()
        * model.var_q_arb["Russia", "M2", _year]()
    )
    if _value > 0:
        print(_year)
...
for _year in model.set_t:
    for _market in model.set_market:
        for _exporter in model.set_e:
            _value = model.var_q_arb[_exporter, _market, _year]()
            if _value > 0:
                print(_year)
                print(_market)
                print(_exporter)
                print(model.var_q_arb[_exporter, _market, _year]())
                print(model.var_mu_12[_exporter, _market, _year]())
                print("")

print("Lambda 1: {:.3f}".format(model.var_lambda_1["Recycling", 2050]()))
print("Lambda 3: {:.3f}".format(model.var_lambda_3[2050]()))
print("Mu 1: {:.3f}".format(model.var_mu_1["Recycling", 2050]()))
print("Mu 12: {:.3f}".format(model.var_mu_12["Recycling", "M2", 2050]()))
