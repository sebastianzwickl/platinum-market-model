...
def write_results_to_ext_iamc_format(m, res_dir):
    ...
    input_iamc.to_excel(os.path.join(res_dir, "0_input.xlsx"), index=False)
    ...
    for e in m.set_exporter:
        ...
        output_iamc = write_iamc(
            ...
            _scenario,
            e,
            "LNG|Gasification|Utilization",
            "%",
            m.year,
            np.around(_utilization, 1),
        )
    ...
    output_iamc.to_excel(os.path.join(res_dir, "1_primal solution.xlsx"), index=False)
    return output_iamc


def write_iamc_with_marginal_exporter(
    output_df, model, scenario, region, variable, unit, time, values, exporter
):
    if isinstance(values, list):
        _df = pd.DataFrame(
            {
...
def write_dual_variables_to_output_files(m, res_dir):
    ...
    for importer in m.set_importer:
        ...
        _price = 0
        _value = m.dual[
            m.con_demand_balance[importer]
        ]  # dual variable of importer's balance constraint
        for exporter in m.set_exporter:
            _des = m.par_des[exporter, importer]

            if (_value <= _des * 1.05) and (_value >= _des * 0.95):
                _marginal_exporter = (
                    exporter  # find marginal exporter per import country
                )
                _check = 1
            else:
                pass
        ...
        _df.to_excel(os.path.join(res_dir, "2_dual solution.xlsx"), index=False)
    return
