...
def comp_13_a(m, t):
    return m.var_mu13[t] <= m.var_u13[t]
...
def comp_14_a(m, t):
    return m.var_mu14[t] <= m.var_u14[t]
    

def comp_14_b(m, t):
    return 0 <= 5 * m.par_demand['M1', t] - m.var_stock_stored[t]


def comp_14_c(m, t):
    return 5 * m.par_demand['M1', t] - m.var_stock_stored[t] <= m.par_big_m * (1 - m.var_u14[t])
...
model.var_market_price_snake = py.Var(
    model.set_market, model.set_t, domain=py.NonNegativeReals
)
model.var_sigma_snake = py.Var(
    model.set_e_stroke, model.set_market, model.set_t, domain=py.Binary
)
...
def upper_limit_of_production_cost(model, market, t):
    return model.var_c_1[market, t] <= 1.25 * model.par_c_gen_e_t['Recycling']


model.eq_upper_limit_of_production_cost = py.Constraint(model.set_market, model.set_t, rule=upper_limit_of_production_cost)


""" MARKET PRICE DETERMINATION """

def determine_market_price(model, m, t):
    return (
        model.var_market_price_snake[m, t]
        == sum(model.var_sigma[e, m, t] * model.par_c_gen_e_t[e] for e in model.set_e_stroke)
    )


model.eq_determine_market_price = py.Constraint(model.set_market, model.set_t, rule=determine_market_price)


def get_active_exporter(model, e, m, t):
    return model.var_q[e, m, t] >= model.var_sigma_snake[e, m, t]


model.eq_get_active_exporter = py.Constraint(
    model.set_e_stroke, model.set_market, model.set_t, rule=get_active_exporter
)


def sum_sigma(model, m, t):
    return sum(model.var_sigma[e, m , t] for e in model.set_e_stroke) == 1
    

model.eq_sum_sigma = py.Constraint(model.set_market, model.set_t, rule=sum_sigma)


def ensure_marginal_exporter(model, e, m, t):
    return model.var_sigma_snake[e, m, t] >= model.var_sigma[e, m, t]


model.eq_ensure_marginal_exporter = py.Constraint(model.set_e_stroke, model.set_market, model.set_t, rule=ensure_marginal_exporter)


""" LINEARIZATION OF THE OBJECTIVE FUNCTION """

model.par_beta_large = py.Param(initialize=10e6)


model.var_z = py.Var(
    model.set_e_stroke, model.set_market, model.set_t, domain=py.NonNegativeReals
)


def eq_a(model, e, m, t):
    return model.var_z[e, m, t] <= model.par_beta_large * model.var_sigma[e, m, t]


def eq_b(model, e, m, t):
    _mexp = model.set_mexporter.first()
    return model.var_z[e, m, t] <= model.var_q[_mexp, m, t]


def eq_c(model, e, m, t):
    _mexp = model.set_mexporter.first()
    return (
        model.var_z[e, m, t]
        >= model.var_q[_mexp, m, t]
        - (1 - model.var_sigma[e, m, t]) * model.par_beta_large
    )


model.eq_a = py.Constraint(model.set_e_stroke, model.set_market, model.set_t, rule=eq_a)
model.eq_b = py.Constraint(model.set_e_stroke, model.set_market, model.set_t, rule=eq_b)
model.eq_c = py.Constraint(model.set_e_stroke, model.set_market, model.set_t, rule=eq_c)
...
solver.options['ImproveStartTime'] = 400
solver.options['ImproveStartGap'] = 0.01

solver.options['ScaleFlag'] = 2
solver.options['NumericFocus'] = 3

solver.options['TimeLimit'] = 5 * 60 * 60
...
for market in model.set_market:
    ...
    _df['Market price'] = [model.var_market_price_snake[market, _year]() for _year in  model.set_t]
...
print("MIP Gap: {:.2f}".format(_mip_gap))
