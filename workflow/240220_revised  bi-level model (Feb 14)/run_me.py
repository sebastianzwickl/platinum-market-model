...
model.par_beta_add = py.Param(initialize=0.05)
...
def der_q_stock_stored(m, t):
    if t == m.set_t.first():
        return (
            m.par_c_stock - m.var_lambda_3[t + 1] + m.var_lambda_4 - m.var_mu7[t] == 0
        )
    elif t == m.set_t.last():
        return m.par_c_stock + m.var_lambda_3[t] - m.var_lambda_7 - m.var_mu7[t] == 0
    else:
        return (
            m.par_c_stock + m.var_lambda_3[t] - m.var_lambda_3[t + 1] - m.var_mu7[t]
            == 0
        )
...
def der_q_stock_in(m, t):
    if t == m.set_t.first():
        return m.var_lambda_1[t] - m.var_lambda_3[t + 1] - m.var_mu9[t] == 0
    elif t == m.set_t.last():
        return m.var_lambda_1[t] - m.var_lambda_8 - m.var_mu9[t] == 0
    else:
        return m.var_lambda_1[t] - m.var_lambda_3[t + 1] - m.var_mu9[t] == 0
